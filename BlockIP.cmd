...
wevtutil qe Security /c:1 /rd:true /f:text /q:"*[System[EventID=4625 and TimeCreated[timediff(@SystemTime) <= 300000]]]" | find "Source Network Address:" > output.tmp
for /f "tokens=4" %%a in (output.tmp) do (set ip=%%a)
echo IP: [%ip%]
...
wevtutil qe Security /c:3 /rd:true /f:text /q:"*[System[EventID=4625 and TimeCreated[timediff(@SystemTime) <= 1800000]]] and *[EventData[Data[@Name='IpAddress'] and (Data='%ip%')]]" | find /c "%ip%" > output.tmp
for /f "tokens=1" %%a in (output.tmp) do (set count=%%a)
echo Count: [%count%]
...
netsh advfirewall firewall show rule name=BlockIP | find "RemoteIP:" > output.tmp
for /f "tokens=2" %%a in (output.tmp) do (set ipblocklist=%%a)
